package edu.wctc.eligrow.football;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int c=0;c <= Math.ceil(score/8); c++) {
            for (int x=0;x <= Math.ceil(score/7); x++) {
                for (int t=0;t <= Math.ceil(score/6); t++) {
                    for (int f=0;f <= Math.ceil(score/3); f++) {
                        for (int s=0;s <= Math.ceil(score/2); s++) {
        ...
        System.out.println(( (double)(finishm-reference) )/1000000000.0 + " Seconds for a score of " + score);
    }
